...
class PulseAudioSinks(PulseAudioBase):
    ...
    async def click_event(self, evt):
        try:
            if evt.name == self.main_block.name:
                ...
                sink = await self.pulse.get_sink_by_name(evt.name)
                if evt.button == MouseButton.wheel_up:
                    await self.pulse.volume_change_all_chans(sink, .05)
                elif evt.button == MouseButton.wheel_down:
                    await self.pulse.volume_change_all_chans(sink, -.05)
                elif evt.button == MouseButton.left:
                    await self.pulse.sink_default_set(sink)
                    await self.pulse.mute(sink, mute=False)
                elif evt.button == MouseButton.right:
                    await self.pulse.volume_set_all_chans(sink, 0)
                else:
    ...
    async def init(self):
        ...
        self.process_vals(await self.pulse.sink_list())
